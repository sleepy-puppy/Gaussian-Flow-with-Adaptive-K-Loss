...
class GaussianFlow(BaseModel):
    ...
    class Config(BaseModel.Config):
        lambda_param_l1: float = 0
        lambda_knn: float = 0.0001
        # YAML의 densify_stop_iter와 일치하도록 기본값 설정
        after_densifi_step: int = 15000
    cfg: Config
    ...
    def get_loss_dict(self, render_results, batch, step: int) -> dict:
        self.global_step = step
        
        # 1. 시간 정보(t) 추출 (forward 함수에 있는 로직과 동일)
        # batch 데이터에서 현재 프레임의 시간 정보를 가져옵니다.
        frame_idx_list = [batch[i]["frame_idx"] for i in range(len(batch))]
        # camera_model을 통해 시간 값(t)을 얻습니다.
        time_input = self.training_camera_model.get_time(frame_idx_list)
        ...
        if self.cfg.lambda_param_l1 > 0:
            param_l1 = self._params_l1_regulizer()
            loss_dict.update({
                "pl1_loss": param_l1,
            })
        ...
        if self.cfg.lambda_knn > 0:
            if self.global_step == self.cfg.after_densifi_step:
                print('knn generation')
                self.point_cloud.gen_knn()
                
            if self.global_step > self.cfg.after_densifi_step:
                # 2. knn_loss 호출 시 time_input 전달
                knn_loss = self.point_cloud.knn_loss(time_input)
